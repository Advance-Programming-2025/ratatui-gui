...
fn render_intro_text(frame: &mut Frame, area: Rect) {
    ...
    let p = Paragraph::new(intro)
        .wrap(Wrap { trim: true })
        .alignment(Alignment::Center)
        .block(Block::default().title(" Mission Briefing ").title_alignment(Alignment::Center));
...
fn render_generation_menu(app: &App, frame: &mut Frame, area: Rect) {
    ...
    let random_style = if is_random { Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::Gray) };
    let custom_style = if is_custom { Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default().fg(Color::Gray) };
    ...
    let menu = Paragraph::new(menu_items)
        .alignment(Alignment::Center)
        .block(Block::bordered().title(" Galaxy Generation Mode ").border_style(Style::default().fg(Color::DarkGray)));
